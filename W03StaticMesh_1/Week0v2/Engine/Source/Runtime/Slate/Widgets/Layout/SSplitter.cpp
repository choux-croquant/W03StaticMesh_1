...
void SSplitterH::LoadConfig(const TMap<FString, FString>& config)
{
    ...
    Rect.leftTopX = GetValueFromConfig(config, "SplitterH.X", GEngineLoop.EditorEngine->graphicDevice.screenWidth *0.5f);
    ...
    Rect.leftTopX *= GEngineLoop.EditorEngine->graphicDevice.screenWidth /GetValueFromConfig(config, "SplitterV.Width", 1000.0f);
}
...
void SSplitterV::LoadConfig(const TMap<FString, FString>& config)
{
    ...
    Rect.leftTopY = GetValueFromConfig(config, "SplitterV.Y", GEngineLoop.EditorEngine->graphicDevice.screenHeight * 0.5f);
    ...
    Rect.leftTopY *= GEngineLoop.EditorEngine->graphicDevice.screenHeight / GetValueFromConfig(config, "SplitterH.Height", 1000.0f);
