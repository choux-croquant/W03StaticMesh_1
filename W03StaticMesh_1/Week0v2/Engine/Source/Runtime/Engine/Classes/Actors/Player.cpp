...
void AEditorPlayer::Input()
{
    ...
    if (GetAsyncKeyState(VK_LBUTTON) & 0x8000)
    {
        if (!bLeftMouseDown)
        {
            ...
            uint32 UUID = GetEngine().EditorEngine->graphicDevice.GetPixelUUID(mousePos);
            ...
            ScreenToClient(GetEngine().EditorEngine->hWnd, &mousePos);

            FVector pickPosition;

            const auto& ActiveViewport = GetEngine().EditorEngine->GetLevelEditor()->GetActiveViewportClient();
...
{
    D3D11_VIEWPORT viewport = GetEngine().EditorEngine->GetLevelEditor()->GetActiveViewportClient()->GetD3DViewport();
    ...
    if (GetEngine().EditorEngine->GetLevelEditor()->GetActiveViewportClient()->IsOrtho())
    {
...
int AEditorPlayer::RayIntersectsObject(const FVector& pickPosition, USceneComponent* obj, float& hitDistance, int& intersectCount)
{
	...
	FMatrix viewMatrix = GEngineLoop.EditorEngine->GetLevelEditor()->GetActiveViewportClient()->GetViewMatrix();
    
    bool bIsOrtho = GetEngine().EditorEngine->GetLevelEditor()->GetActiveViewportClient()->IsOrtho();
    ...
    if (bIsOrtho)
    {
        ...
        FVector orthoRayDir = GEngineLoop.EditorEngine->GetLevelEditor()->GetActiveViewportClient()->ViewTransformOrthographic.GetForwardVector().Normalize();
...
void AEditorPlayer::ControlTranslation(USceneComponent* pObj, UGizmoBaseComponent* Gizmo, int32 deltaX, int32 deltaY)
{
    ...
    auto ActiveViewport = GetEngine().EditorEngine->GetLevelEditor()->GetActiveViewportClient();
